...
/**
 * 添加list数据
 * 注意：必须同时输入setList和adapter两个属性，否则编译出错
 */
@BindingAdapter(value = ["addList", "adapter"], requireAll = true)
fun <T> RecyclerView.addList(list: MutableList<T>?, inputAdapter: RecyclerView.Adapter<*>?) {
    inputAdapter?.let {
        try {
            if (inputAdapter is BaseRecyclerViewAdapter<*,*>) {
                (inputAdapter as BaseRecyclerViewAdapter<T,*>).addItemdataList(list)
 ...
 * 注意：必须同时输入setList和adapter两个属性，否则编译出错
 */
@BindingAdapter(value = ["setList", "adapter"], requireAll = true)
fun <T> RecyclerView.setList(list: MutableList<T>?, inputAdapter: RecyclerView.Adapter<*>?) {
    inputAdapter?.let {
        try {
            if (inputAdapter is BaseRecyclerViewAdapter<*,*>) {
                (inputAdapter as BaseRecyclerViewAdapter<T,*>).clear()
                (inputAdapter as BaseRecyclerViewAdapter<T,*>).addItemdataList(list)
                adapter = inputAdapter
            }
        } catch (e: Exception) {
