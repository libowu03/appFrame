...
import com.base.net_lib.callback.BasePostInterface
 ...
 * post请求的对外类
 */
class NetPostRequest<T>(url: String, okHttpClient: OkHttpClient?) :
    BaseNetRequest<T>(url, okHttpClient), BasePostInterface<T> {
     ...
     * @param value 值
     */
    override fun form(key: String, value: String?): NetPostRequest<T> {
        value?.let {
            mForm.add(key, value)
        }
        return this
     ...
     */
    override fun json(value: Any?, uploadType: String):NetPostRequest<T>{
        ...
        try {
            ...
            L.e(NET_HTTP_ERROR, e.localizedMessage)
        }
        jsonStr?.let {
     ...
     * @param path 上传文件所在地址
     */
    override fun file(path: String?, uploadType: String):NetPostRequest<T>{
        val file = File(path)
        if (!file.exists()) {
            L.w(NET_HTTP_WARN, "“${path}”文件不存在，请确认")
            return this
        }
     ...
     * 上传混合表单
     */
    override fun multipartBody(body: MultipartBody?):NetPostRequest<T> {
        body?.let {
    ...
    override fun getRequest(): Request? {
        ...
        mRequest?.url(mUrl)
        mRequest?.post(mForm.build())
        mRequest?.post(mMultipartBody.build())
